...
//		AfxGetMainWnd()->GetMenu()->CheckMenuItem(ID_SIZE_D1,MF_CHECKED);
		...
		CString RootPath="C:\\data";



		PicSavePath = RootPath+"\\"+time.Format("%Y%m%d-%H%M%S");
		PicPath = PicSavePath+"\\"+"Pic";
		
		RecSavePath = RootPath+"\\"+time.Format("%Y%m%d-%H%M%S");
...
//		CString StrPic; 
//
//		GetPrivateProfileString("SystemSetting","PicSaveParh","D:\\Data"+time.Format("%Y%m%d-%H%M%S"),StrPic.GetBuffer(128),128,strIniFile);
...
void CDlgDemoDlg::ReceiveDataProcess()
{
	...
	//判断是否准备充电
	if(ReceiveData.allData.actionCtrl==0x10 || ReceiveData.allData.actionCtrl==0x08)   //包含测试代码
	{
		ReadyClosePort=0x01;
	}
