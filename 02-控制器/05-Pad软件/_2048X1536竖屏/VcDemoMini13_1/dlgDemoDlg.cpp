...
void CDlgDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_EDIT, m_ctrlEditReceive);
...
//////////////////////////////////////打开相关设备//////////////////////////////////////////////
	...
	//打开定时器2，用于定时发送数据
	SetTimer(2,300,NULL);//启动定时器1,定时时间是0.1秒
	//打开定时器3，用于定时更新电量信息
...
//将收到的数据保存在ReceiveData中
//	ReceiveData[number]=ch;
//	if(number==6)
//		number=0;
//	else
//		number++;
	...
	if(ch==0xA5 || isHeadEntry)      //如果是帧头或者帧头已经入队，数据才可以入队，否则一概不能入队。
	{
		isHeadEntry=true;     //标记帧头已经入队
		ReceiveQueue.EnQueue((char)ch);
	}
	
	if(ReceiveQueue.IsFull())  //如果队满，就全部出队
	{
		//	AfxMessageBox("isFull");
		...
		m_strReceive.Format("%X %X %x %x %x %x %x %x ",
	...
	static int isSnap=0;
//判断是否拍照
	if(ReceiveData.allData.action==0x08)
	{
		if(isSnap==0)
		{
			OnButtonSnap();	
...
//判断是否在录像
	...
	if(ReceiveData.allData.action==0x04)
	{
		if(isRecord==0)
		{
			OnBtRecord();	
...
//		
//		//先将高4位和低4位分离
//		high4=ReceiveData.Data[j]/16;      //2^4=16
//		low4=ReceiveData.Data[j]%16;
//		
//		
//		
//		
...
void CDlgDemoDlg::SendSerialData()
{
	...
	for(int i=0;i<7;i++)
	{
		strSendData+=SendData[i];
	}

//strSendData+=65;
//strSendData+=66;
	m_SerialPort.WriteToPort(SendData,7);//发送数据
	//AfxMessageBox(strSendData);
}
