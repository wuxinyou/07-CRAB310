...
//		AfxGetMainWnd()->GetMenu()->CheckMenuItem(ID_SIZE_D1,MF_CHECKED);
   ...
   //end of menu set
	...
	//	WritePrivateProfileString("SystemSetting","VideoSaveParh",RecSavePath,strIniFile);
		...
		CString StrPic; 
		GetPrivateProfileString("SystemSetting","PicSaveParh","C:\\Pic",StrPic.GetBuffer(128),128,strIniFile);
		StrPic.ReleaseBuffer();
		PicSavePath=StrPic;
			//get the save picture file path form ini file 
		CString StrVid; 
		GetPrivateProfileString("SystemSetting","VideoSaveParh","C:\\Video",StrVid.GetBuffer(128),128,strIniFile);
...
//	}
	...
	////////////////////////////////////////read inf file end//////////////////
		...
		if(m_nDevNum==1) 
		{
				::MoveWindow(GetDlgItem(IDC_ST_DISPLAY)->GetSafeHwnd(),0,0,800,	576,false);
		}
...
//	UpdateData();
	...
	if ( m_nDevNum == 1 )
	{
	//改变dwDispWidth和dwDispHeight可以改变预览窗口的大小

		DeviceInfo[dwCard].dwDispWidth = 720;
		DeviceInfo[dwCard].dwDispHeight = 576;
	//dwCard是窗口号，dwCard从0开始计数，其实对于一个预览窗口而言，dwCard=0；
