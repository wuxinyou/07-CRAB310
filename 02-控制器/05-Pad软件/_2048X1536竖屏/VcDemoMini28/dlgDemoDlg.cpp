...
void CDlgDemoDlg::DoDataExchange(CDataExchange* pDX)
{
	...
	DDX_Control(pDX, IDC_RECICON, m_RECIcon);
...
void CDlgDemoDlg::OnTimer(UINT nIDEvent) 
{
	...
	//		m_RECIcon.ShowWindow(SW_SHOW);
		}
		else
		{
			DrawRecordFrame(RGB(0,250,0));
			m_RECIcon.ShowWindow(SW_HIDE);
		}
		flag=!flag;
		break;
	case 2:
...
void CDlgDemoDlg::InitWidget()
{
	...
	// 设置电量图标部件位置和尺寸
	this->m_padVol.MoveWindow(112,17,70,20);
	this->m_ctrlBoxVol.MoveWindow(112,45,96,16);
	this->m_leftVol.MoveWindow(352,17,96,16);
	this->m_rightVol.MoveWindow(352,45,96,16);
...
//初始化预览区窗口	
	...
	//初始化充电动态图 和 加载资源动态图
	m_chargeGIF.MoveWindow(VideoArea_X,VideoArea_Y,VideoArea_W,VideoArea_H);
	m_loadDevicesGIF.MoveWindow(VideoArea_X,VideoArea_Y,VideoArea_W,VideoArea_H);
...
//	//REC图标 按钮 设置
//	m_RECIcon.SetAlign(2);
//	m_RECIcon.SetIcon(IDI_SNAPBTN1,IDI_SNAPBTN1);
//	m_RECIcon.SetFlat(true);
//	m_RECIcon.DrawTransparent(TRUE);
//	m_RECIcon.MoveWindow(40,130,134,139);
	...
	m_snapBtn.MoveWindow(0,816,134,139);
	...
	m_recordBtn.MoveWindow(134,816,134,139);
	...
	m_settingBtn.MoveWindow(268,816,134,139);
	...
	m_openBtn.MoveWindow(402,816,134,139);
	...
	m_txtViewerBtn.MoveWindow(536,816,134,139);
	...
	m_picViewerBtn.MoveWindow(670,816,134,139);
	...
	m_videoPlayerBtn.MoveWindow(402,816,134,139);
