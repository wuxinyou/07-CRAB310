...
参数：
	device - [in] 设备句柄；
	source - [in] 当前选择的输入端子的序号,设备为UVC：0为HDMI，1为SDI；2为PATTERN；
返回值：
	0 - 成功；失败则返回错误代码
// */
...
unsigned __stdcall DXStartCapture(device_handle device, TCHAR* szFileName, BOOL saveAudio,
										  ...
										  ISaveNotify* saveNotify = NULL,unsigned vidRatio=1);
...
返回值：
	0 - 成功；失败则返回错误代码
// */
unsigned __stdcall DXStartCaptureQY(device_handle device, TCHAR* szFileName, BOOL saveAudio,RECT *rct = NULL, unsigned vidRatio=1);
...
返回值：
	0 - 成功；失败则返回错误代码
// */
unsigned __stdcall DXSetEncAudioCallback(device_handle device, fnEncAudioCallback fn, void* context);
...
返回值：
	0 - 成功；失败则返回错误代码
// */
unsigned __stdcall DXEnFScale(device_handle device,bool bEnFScale, RECT* rect, SwsAlogrithm enAlogrithm=SWS_SA_FAST_BILINEAR);
