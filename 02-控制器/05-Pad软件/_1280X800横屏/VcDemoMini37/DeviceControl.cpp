...
BOOL DeviceControl:: StartRecord(HANDLE ChannelHandle,CString szFileName,CString CodecType, VidCodecPara nCodecPara)
{
		...
		if (strcmp(XVIDCODEC_FILTER,CodecType)==0)
		{
			DXSetVideoCodecPara(ChannelHandle, codec_xvid, &nCodecPara);	
		}


		ok=DXStartCapture(ChannelHandle, TEXT(szFileName.GetBuffer(szFileName.GetLength())), FALSE, NULL, NULL, NULL,1);
