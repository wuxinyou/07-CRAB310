...
void CDlgDemoDlg::OnBtRecord() 
{
	...
	{	
		...
		if(!RecOK)
		{
			...
			this->DrawRecordFrame(RGB(250,0,0));//按下后立即绘制红色边框
			IsRecord=true;
...
void CDlgDemoDlg::OnTimer(UINT nIDEvent) 
{
	...
	case 3:   //time=1000
		showVoltagePicture();
		ShowBluetooth();
		//	m_bSerialPortConnected=false;  //这里的目的是为了没1000ms，更新一次显示，如果OnCOM收到数据后，会标记true
...
//				OnOpendevbtn(2);			
	...
	case 5:   //time=100
		...
		//	ShowCompass();
		//由于去除了显示画面切换状态，所以不需要跟新OSD了，至于时间OSD，采集卡会自动更新
		//	ShowOSD();
...
#ifdef USE_SDK
	if(!bDevIsConnect)  //如果采集卡没有连接上，则继续初始化采集卡
	{
		DXUninitialize();
		InitSDK();
...
#ifdef USE_SDK
	if(!m_bSerialPortOpened && bDevIsConnect)   //如果采集卡连接上了，而且串口没有有打开，则继续打开串口
#else
		if(!m_bSerialPortOpened)
#endif
		{
...
int CDlgDemoDlg::Voltage2Volume(unsigned char voltage,int cellNum)
{
	...
	singleCellVoltage=voltage*0.1/cellNum;
...
void CDlgDemoDlg::showVoltagePicture()
{
	...
	int ctrlBoxPower = ReceiveData.allData.ctrlBoxPower;    //不做处理，直接赋值
	...
	if(m_bSerialPortConnected)
	{
		...
		else if(ctrlBoxPower<113)
		{
			hBitmap = LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_BITMAP40)); 
			m_ctrlBoxVol.SetBitmap(hBitmap);                 /***IDB_BITMAP1是你载入资源中位图的ID号***/	
		}
		else if(ctrlBoxPower<115)
		{
			hBitmap = LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_BITMAP60)); 
			m_ctrlBoxVol.SetBitmap(hBitmap);                 /***IDB_BITMAP1是你载入资源中位图的ID号***/
		}
		else if(ctrlBoxPower<=120)
		{
			hBitmap = LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_BITMAP80)); 
			m_ctrlBoxVol.SetBitmap(hBitmap);                 /***IDB_BITMAP1是你载入资源中位图的ID号***/
		}
...
{
	BOOL bluetoothStatus=m_bSerialPortConnected;
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	// 设置电量图标部件位置和尺寸
	this->m_padVol.MoveWindow(764,14,76,22);
	this->m_ctrlBoxVol.MoveWindow(993,14,76,22);
