...
//		vbtCtr->setOff();
	...
	if(AD_Filter[NUM_KEY_POWER]>=0X240&&ctrFlag_PowerOn==1)
	{
		...
		if(powerOffHardWareCountDown<POWER_OFF_HARD_WARE_COUNT_DOWN/2)
		{
			powerOffHardWareCountDown++;
			//按键延时一小段时间后，发送关机指令，这个时间比强制关机时间短
			if(powerOffHardWareCountDown > POWER_OFF_HARD_WARE_COUNT_DOWN/4)
			{
					ctrData1&=~(0x03<<6);
		      ctrData1|=0x01<<6;
			}
 ...
 Process::Process()
{
	...
	initAllData();														//	初始化所有数据
	testLed->setOn();													//	
	SysTick_DelayMs(1000);												//	按下电源开关3000ms后再开机，数传需要初始化2s左右
