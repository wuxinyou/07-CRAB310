...
void VideoPlayerDlg::InitWidget()
{
	...
	this->MoveWindow(106,85,1026,636);
	m_ctrlPic.MoveWindow(0,0,720,576);
	m_ctrlList1.MoveWindow(720,0,300,302);
	m_ctrlList2.MoveWindow(720,304,300,302);
	m_ctrlSliderPlayTime.MoveWindow(0,580,720,30);
	...
	//显示OD Log
	m_ctrlODLogPic.MoveWindow(0,0,720,576);
...
void VideoPlayerDlg::FullScreenShow()
{
    ...
    if ((TRUE == m_bOpen) && (FALSE == m_bFullScreen))
    {
        ...
        //Picture控件全屏显示
        GetDlgItem(IDC_VIDEOPLAYER_PIC)->MoveWindow(CRect(0, 0, 1280, 800));
...
void VideoPlayerDlg::NormalShow(void)
{
    if ((TRUE == m_bOpen) && (TRUE == m_bFullScreen))
    {
		...
		{
			m_ctrlODLogPic.MoveWindow(0,0,720,576);
			ShowODLogPic(CRect(0,0,720,576));	
		}
		else
		{
			m_ctrlODLogPic.ShowWindow(SW_HIDE);	
...
void VideoPlayerDlg::OnPaint() 
{
	...
	if ( m_bFullScreen) {
		...
		dc.FillRect( &rect_client, &brush_fill_rect );
