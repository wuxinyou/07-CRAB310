...
{
	//	1101 pvwMask->ShowWindow(SW_SHOW);
...
void CDlgDemoDlg::showVoltagePicture()
{
	...
	{			
		hBitmap = LoadBitmap(AfxGetInstanceHandle(),MAKEINTRESOURCE(IDB_BITMAP0)); 
		m_ctrlBoxVol.SetBitmap(hBitmap);                 /***IDB_BITMAP1是你载入资源中位图的ID号***/
...
#else
	m_loadDevicesGIF.ShowWindow(SW_HIDE);  //1101
...
{
	CDlgDemoDlg* m_pMainDialog = (CDlgDemoDlg*)pUserData;
...
BOOL CDlgDemoDlg::HwUninitialize()
{	
	if ( m_hCloneCapDev != 0 ) {
...
void CDlgDemoDlg::InitPD652()
{
	...
	m_oChannelWindows[ 0 ].Create( NULL, "MyChannelWindow#01", WS_CHILD | WS_VISIBLE, 
		CRect(VideoArea_X, VideoArea_Y, VideoArea_W, VideoArea_H), this, 0x00000001, NULL ); // VIDEO WINDOW
	
	m_oChannelWindows[ 1 ].Create( NULL, "MyChannelWindow#01", WS_CHILD | WS_VISIBLE, 
		CRect( VideoArea_X, VideoArea_Y, VideoArea_W, VideoArea_H), this, 0x00000002, NULL ); // BACKGROUND WINDOW
	
	m_oCloneWindows.Create( NULL, "MyCloneWindow#01", WS_CHILD | WS_VISIBLE, 
		CRect( VideoArea_X, VideoArea_Y, VideoArea_W, VideoArea_H), this, 0x10000001, NULL ); // BACKGROUND WINDOW
	...
	//
	{
		...
		//	MoveWindow( 100, 100, cx + 720, cy + 480 );
		//	MoveWindow( 100, 100, 1220, 880 );
		
		
//		CString debug;
//		debug.Format("%d  %d",cx,cy);
//		AfxMessageBox(debug);
		
		
		m_oChannelWindows[ 0 ].MoveWindow( VideoArea_X, VideoArea_Y, VideoArea_W, VideoArea_H);
		//
		m_oChannelWindows[ 1 ].MoveWindow( VideoArea_X, VideoArea_Y, VideoArea_W, VideoArea_H);
...
///////////////////////////////////////录像和拍照////////////////////////////////
//拍照
BOOL CDlgDemoDlg::OnButtonSnapshotJPG() 
{
	if ( m_hVideoDevice != 0 ) 
	{
		...
		ULONG nStrlen = strPath.GetLength();

		CHAR* psz = strPath.GetBuffer( nStrlen+1 );
...
//录像开始
BOOL CDlgDemoDlg::OnButtonRecord()
{
	//	m_oSetupDialog.m_btnRecordStart_1_1.EnableWindow( FALSE ); 
	//
	...
	if ( m_hVideoDevice != 0 ) 
	{
		if(m_bIsRecord1==false)
		{
			...
			CTime time = CTime::GetCurrentTime();
			strFullPathName = VideoPath+"\\" + time.Format("%Y%m%d-%H%M%S.mp4");
		...
		{
			this->DrawRecordFrame(RGB(0,0,0));   //绘制黑色边框
			KillTimer(1);        //关闭定时器1。
...
//录像结束
//BOOL CDlgDemoDlg::OnButtonRecordStop1()
//{
//	m_oSetupDialog.m_btnRecordStart_1_1.EnableWindow( TRUE ); 
//
	//	m_oSetupDialog.m_btnRecordStop_1_1.EnableWindow( FALSE ); 
