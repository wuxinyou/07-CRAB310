...
{		//				 序号   ,位置x  ,高度   ,位置y  ,半径   ,角度  ,大小
	...
	glRotatef(m_rotateAngleX,1.0, 0, 0);	//整体坐标绕X旋转
	m_rotateAngleY=180;
	glRotatef(m_rotateAngleY,0, 0, 1.0);	//整体坐标绕Y旋转
...
//{		//				 序号   ,位置x  ,高度   ,位置y  ,半径   ,角度  ,大小
			...
			Scene(1,0,0,-1.25,0,m_angle,0.4f);//显示对象
			Scene(0,0,0,-1.25,0,0,0.5f);//显示对象
...
//			{
//				flag=0;
//				AfxMessageBox("Render");
//			}
			

		glDisable(GL_TEXTURE_2D);

		//	m_angle+=3;if(m_angle>360) m_angle=0;
...
void C3DSShow::InitCWnd()
{
	...
	Width=130;//GetSystemMetrics(SM_CXSCREEN);
				Height=130;//GetSystemMetrics(SM_CYSCREEN);
				MoveWindow(17,621,Width,Height);//窗口定位
				////////////////////////////////////////////////////////////
				glViewport(0, 0,Width,Height);// reset the viewport to new dimensions
				glMatrixMode(GL_PROJECTION);	// set projection matrix current matrix
				gluPerspective(25.0f,(GLfloat)Width/(GLfloat)(Height-0),1.0f,2000.0f);
				glMatrixMode(GL_MODELVIEW);		// set modelview matrix
				glLoadIdentity();				// reset modelview matrix
				glEnable(GL_TEXTURE_2D);	// 启用纹理映射(新增)
				glShadeModel(GL_SMOOTH);
				...
				load3dobj("data/","CRAB_FAN2.3DS",1);
...
void C3DSShow::SetRotateAngle(int angle)
{
	m_angle=angle;
}
...
void C3DSShow::OnMouseMove(UINT nFlags, CPoint point) 
{
	...
	{		
		...
		setRotateParm( (float)cy /10 ,(float)cx/30 );
