...
VcDemoMini51
20160922
1.修复：图片查看器列表1不能自动添加滚动条
	解决：将列表1加载代码放在列表布局后面。

VcDemoMini52
20161020
1.添加：图片查看器双击全屏
2.图片加载使用的是CPictureEx类，所以需要解决的问题就是CPictureEx类加载图片和缩放的问题
3.参照了网上的相关介绍：http://www.xuebuyuan.com/1494713.html
	基本完成了图片的双击实现全屏切换。	
4.原始的想法是不是图片加载或者图片显示的时候对图片的尺寸有什么参数可以选择，实际上CPictureEx类已经考虑到了缩放的问题
	所以只需要修改：OnPaint函数，这个方法里使用了BitBlt函数，这是一个不能缩放图片的函数，我们使用StretchBlt来实现缩放。	
	另外在需要缩放的时候调用：使用SetPaintRect函数，此函数在CPictureEx类中。
5.其他参照：http://www.cppblog.com/javen/archive/2007/03/14/19852.html
			http://blog.csdn.net/kkk3108001227/article/details/6568142
6.另外：由于PicViewer的窗口背景（也就是公司Logo图片）和加载的图片是公用一个CPictureEx部件，
	所以这里一并解决了窗口背景双击全屏的问题。

VcDemoMini53
20161027
1.完善视频回放界面的ODLogPic全屏显示功能。
2.设置总在最前面：软件启动时默认为总在最前面，当关闭进程VLC之后，恢复不在最前面。
*************************************************************************************************************
*************************************************************************************************************
PD652_02
20161031
1.本实例是VcDemoMini53版本的修订版，PD652_02结合了VcDemoMini53的所有界面功能除了采集卡的操作，
	同时结合了PD652_01的采集卡功能。实际上是将PD652_01的采集卡功能移植到VcDemoMini53中。。。
2.基本了解，程序框架，准备移植。移植方法：将pd652dlg的部分代码在dlgDemoDlg中重写。
	20161101 am
3.采集卡初始化基本成功，可以实现相机预览。
...
2.问题：如何裁剪，
	1.完全裁剪掉TSW采集卡的驱动代码。
	2.保留TSW采集卡的驱动代码，通过宏来条件编译。
3.实现先启动软件后接入采集卡，能够正常连接。

PD652_05
20161102 
1.完全裁剪掉TSW采集卡的驱动代码，暂时没有完全删除裁剪，但是基本隔离了。
2.修改图片浏览窗口和视频回放窗口的尺寸。
3.为视频回放窗口添加OnPaint填充黑色背景功能。
4.发布可以正常使用版本。